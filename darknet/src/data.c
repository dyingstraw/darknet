...
{
    char labelpath[4096];
    //有人一直不知道labels怎么来的，说源码都没设置labels的路径啊，怎么读的labels啊，那不是成了无监督学习？
    //其实源码只是没直接设置labels的路径而已，把images替换为labels，在把.jpg替换为.txt，labels的路径就有了～
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
    ...
    find_replace(labelpath, ".JPG", ".txt", labelpath);
    find_replace(labelpath, ".JPEG", ".txt", labelpath);
    // 一阵操作猛如狗，才发现为什么要把.txt的标签文件放在与图片相同的路径，事实上，作者的初衷可能不是这样的
    int count = 0;
    // 读取box，并记录个数
    box_label *boxes = read_boxes(labelpath, &count);
    // 打乱box的顺序
    randomize_boxes(boxes, count);
    // 因为图像已经被修剪了，所以框的坐标也要改一改，
    // correct_boxes函数就是把框在原始图像下的坐标转到修剪后图像下的坐标
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
...
pthread_t load_data(load_args args)
{
    ...
    *ptr = args;
    ...
    /** 
     * @brief   第一个参数为指向线程标识符的指针；第二个参数用来设置线程属性；
