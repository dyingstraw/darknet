...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "average")){
        ...
        run_lsd(argc, argv);
    } else if (0 == strcmp(argv[1], "detector")){//detector 指令
        run_detector(argc, argv);
    } else if (0 == strcmp(argv[1], "detect")){//这个是官网./darknet detect cfg/yolov3-tiny.cfg yolov3-tiny.weights data/dog.jpg 的入口
        float thresh = find_float_arg(argc, argv, "-thresh", .5);//检测阈值，默认为0.5
        char *filename = (argc > 4) ? argv[4]: 0;
        char *outfile = find_char_arg(argc, argv, "-out", 0);//输出文件的路径
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        //按照这些参数运行检测器测试，这里有一点，cfg/coco.data的路径是固定的，有点尴尬，后期后面看看是作者的失误还是哈原因（2018/07/16:markdown）
