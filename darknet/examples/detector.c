...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    // 如果没有图片列表文件，默认使用data/train.list作为图片列表
    char *train_images = option_find_str(options, "train", "data/train.list");
    // 加载权重备份之地，默认维backup文件夹
    ...
    while(get_current_batch(net) < net->max_batches){
        // cfg最后的random成立，且训练次数是10的整数倍，多尺度变换
        if(l.random && count++%10 == 0){
            ...
            //这里采用多线程的方式，这里获取的train里重要的成员变量有
            // X{rows=batch, cols=[net].w*[net].h*[net].c}
            train = buffer;
            free_data(train);
...
// datacfg: 为.data文件的路径
// cfgfile: 网络配置文件路径
// weightfile: path of XX.weight
// filename: path of the detected image
// thresh: 阈值，默认0.5
// hier_thresh： 还不知道用途（2017/07/16:markdown）
// outfile: 检测结果图片存储路径，默认当前文件夹下
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    // 如果没有定义name参数，则固定使用data/names.list文件作为namelist
    char *name_list = option_find_str(options, "names", "data/names.list");
    // 得到所谓分类的名称
    char **names = get_labels(name_list);
    // 好像是得到png标签图片（2017/07/14：markdown）
...
void run_detector(int argc, char **argv)
{
    ...
    // 看一下训练的原理，应该比较复杂2018/07/17：markdown
